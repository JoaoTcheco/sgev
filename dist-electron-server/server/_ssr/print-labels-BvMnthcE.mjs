import { f as formatMZN } from "./router-DE-fAUtY.mjs";
...
function useLabelSettings() {
  const [settings, setSettings] = reactExports.useState(() => read());
...
function expand(entries) {
  return entries.flatMap(
    (e, ei) => Array.from({ length: Math.max(0, Math.floor(e.qty)) }).map((_, i) => ({
      id: `bc_${ei}_${i}`,
      name: e.name,
      barcode: e.barcode,
      price: e.price ?? null,
...
function printLabels(entries, override) {
  const cfg = { ...getLabelSettings(), ...{} };
...
function renderA4(all, cfg) {
  ...
  `).join("");
  const init = all.map((l) => `JsBarcode(document.getElementById(${JSON.stringify(l.id)}), ${JSON.stringify(l.barcode)}, { format:'CODE128', height:34, width:1.3, fontSize:9, margin:0 });`).join("\n");
...
function renderThermal(all, cfg) {
  ...
  `).join("");
  const init = all.map((l) => `JsBarcode(document.getElementById(${JSON.stringify(l.id)}), ${JSON.stringify(l.barcode)}, { format:'CODE128', height:${Math.round(barcodeHeightMm * 3.78)}, width:1.2, fontSize:${Math.max(6, fontSizePt - 1)}, margin:0, displayValue:true });`).join("\n");
  const printerHint = cfg.printerName ? `<div class="hint">Impressora sugerida: <b>${escapeHtml(cfg.printerName)}</b></div>` : "";
...
export {
  DEFAULT_LABEL_SETTINGS as D,
  printLabels as p,
  useLabelSettings as u
};
