<?php
$statusLabels = ['open'=>'Em aberto','partial'=>'Parcial','paid'=>'Pago','canceled'=>'Cancelada'];
...
<section class="apar-view">
  <div class="crud-header">
    ...
    <div class="header-actions">
      <a href="<?= url('payables') ?>" class="btn btn-ghost">← Voltar</a>
      <?php if ($canPay): ?>
        <a href="<?= url('payables/edit') ?>&id=<?= e($item['id']) ?>" class="btn">Editar</a>
      <?php endif; ?>
    </div>
  </div>
  ...
  <div class="apar-kpis">
    ...
    <div class="apar-tile"><span class="lbl">Pago</span><span class="val"><?= formatMZN($item['paid_amount']) ?></span></div>
  ...
  <?php if ($canPay && $balance > 0): ?>
    <div class="card">
      ...
      <form method="POST" action="<?= url('payables/pay') ?>" class="pay-form">
        <?= csrfField() ?>
        ...
        <div class="grid-4">
          ...
          <label>Método
            <input type="text" name="method" placeholder="Ex: transferência">
          </label>
        </div>
        <label>Notas
        ...
        <div class="form-actions">
          <button class="btn btn-primary">Pagar <?= formatMZN($balance) ?></button>
        </div>
      </form>
    </div>
  <?php endif; ?>
  ...
  <div class="card">
    ...
    <?php if (!$payments): ?>
      <p class="empty">Sem pagamentos.</p>
    <?php else: ?>
      <table>
  ...
  <?php if ($canPay): ?>
    <div class="danger-zone">
      <?php if ((float)$item['paid_amount'] == 0): ?>
        <form method="POST" action="<?= url('payables/cancel') ?>" onsubmit="return confirm('Cancelar esta conta?')" style="display:inline">
          <?= csrfField() ?><input type="hidden" name="id" value="<?= e($item['id']) ?>">
          <button class="btn btn-warning">Cancelar conta</button>
        </form>
        <form method="POST" action="<?= url('payables/delete') ?>" onsubmit="return confirm('Eliminar permanentemente?')" style="display:inline">
          <?= csrfField() ?><input type="hidden" name="id" value="<?= e($item['id']) ?>">
