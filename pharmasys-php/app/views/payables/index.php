...
<section class="ap-ar">
  ...
  <div class="apar-kpis">
    ...
    <div class="apar-tile"><span class="lbl">Pago no mês</span><span class="val"><?= formatMZN($kpis['paid_month']) ?></span></div>
  </div>

  <form method="GET" class="apar-filters">
    <input type="hidden" name="r" value="payables">
    <input type="text" name="q" placeholder="Descrição ou fornecedor…" value="<?= e($filters['q']) ?>">
    <select name="status">
    ...
    <a href="<?= url('payables') ?>" class="btn btn-ghost">Limpar</a>
  </form>
  ...
  <div class="apar-table">
    <table>
      <thead>
        <tr>
          <th>Vencimento</th><th>Fornecedor</th><th>Descrição</th>
          <th class="right">Valor</th><th class="right">Pago</th><th class="right">Saldo</th>
          <th>Estado</th><th></th>
        </tr>
      </thead>
      <tbody>
        ...
        ?>
          <tr class="<?= $isOverdue?'row-overdue':($isSoon?'row-soon':'') ?>">
            ...
            <td><?= e($p['supplier_name'] ?? '—') ?><?php if ($p['po_number']): ?><br><small><?= e($p['po_number']) ?></small><?php endif; ?></td>
            ...
            <td><a href="<?= url('payables/view') ?>&id=<?= e($p['id']) ?>" class="btn btn-sm">Abrir</a></td>
          </tr>
