...
<section class="apar-form">
  ...
  <form method="POST" action="<?= url('payables/save') ?>" class="form-card">
    <?= csrfField() ?>
      ...
      <label>Fornecedor
        <select name="supplier_id">
          ...
          <?php foreach ($suppliers as $s): ?>
            <option value="<?= e($s['id']) ?>" <?= ($item['supplier_id'] ?? '')===$s['id']?'selected':'' ?>><?= e($s['legal_name']) ?></option>
          <?php endforeach; ?>
        </select>
      </label>
      <label>Descrição *
    ...
    <div class="form-actions">
      ...
      <a href="<?= url('payables') ?>" class="btn btn-ghost">Cancelar</a>
    </div>
  </form>
</section>
